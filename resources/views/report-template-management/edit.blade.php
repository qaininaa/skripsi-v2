...
@section('page-title', 'Edit Template Laporan')

@section('content')
    <div
        x-data="{
            mediums: @js(old('medium_templates', $reportTemplate->mediumTemplates->map(fn($m) => ['name' => $m->name])->values()->toArray())),
            incubators: @js(old('incubator_templates', $reportTemplate->incubatorTemplates->map(fn($i) => ['label' => $i->label, 'min_day' => $i->min_day])->values()->toArray())),
            addMedium() {
            ...
            formatIncubatorLabel(index) {
                let raw = this.incubators[index].label.trim();
                if (raw === '') return;
                raw = raw.replace(/\s*°C$/i, '').trim();
                if (raw !== '') {
    ...
    >
        <h1 class="text-xl font-bold text-gray-900">Edit Template Laporan</h1>
        <p class="mt-1 text-sm text-gray-500">Perbarui konfigurasi template laporan.</p>
        ...
        <form action="{{ route('report-templates.update', $reportTemplate) }}" method="POST" class="mt-6 space-y-5">
                    ...
                    <template x-for="(medium, index) in mediums" :key="index">
                        <div class="flex items-center gap-2">
                            ...
                            <button
                                ...
                                class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-500 transition-colors hover:bg-red-100 cursor-pointer"
                                title="Hapus medium"
                            >
                                <img src="{{ asset('icons/trash.svg') }}" alt="Hapus" class="h-4 w-4">
                            </button>
                        </div>
                    </template>
                </div>
                    ...
                    <template x-for="(incubator, index) in incubators" :key="index">
                        <div class="flex items-center gap-2">
                            <div class="relative flex-1">
                                ...
                                >
                                <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center text-xs text-gray-400">°C</span>
                            </div>
                            <input
                                type="number"
            ...
            {{-- Pemantauan Personel --}}
            <div
                x-data="{ enabled: {{ old('has_personnel', $reportTemplate->has_personnel) ? 'true' : 'false' }} }"
