...
@section('content')
    <div class="mx-auto max-w-2xl rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
        ...
        <p class="mt-1 text-sm text-gray-500">Perbarui informasi lokasi pengambilan sampel.</p>

        <x-messages.info-message />
        <x-messages.error-message />
        <x-messages.validation-errors />

        <form action="{{ route('location.update', $location) }}" method="POST" class="mt-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                >
                    ...
                    @foreach ($rooms as $room)
                        <option value="{{ $room->id }}" @selected(old('room_id', $location->room_id) === $room->id)>
            ...
            <div>
                ...
                <input
                    ...
                    id="loc_number"
                    name="loc_number"
                    value="{{ old('loc_number', $location->loc_number) }}"
            ...
            <div>
                ...
                >
                    ...
                    @foreach (['Settle Plate', 'Contact Plate', 'Air Sampler', 'Swab'] as $type)
                        <option value="{{ $type }}" @selected(old('measurement_type', $location->measurement_type) === $type)>
                            {{ $type }}
                        </option>
                    @endforeach
                </select>
